...
def encrypt_decrypt_folder(folder):
...
if len(sys.argv) == 2:
    ...
    folder = sys.argv[1]
    encrypt_decrypt_folder(folder)
elif ((len(sys.argv) < 2) or (sys.argv is None or " ")):
    # Get folder from user input
    folder = input("Folder to encrypt: ")
    encrypt_decrypt_folder(folder)
elif (len(sys.argv) > 2):
    print("This script takes only one parameter (folder name).")
